drop function if exists public.mt_upsert_locatiezonderadresmatchdocument(JSONB, varchar, varchar, integer) cascade;
CREATE OR REPLACE FUNCTION public.mt_upsert_locatiezonderadresmatchdocument(doc jsonb, docdotnettype character varying, docid character varying, revision integer)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
DECLARE
...
END;
$function$;
drop function if exists public.mt_upsert_powerbiexportdocument(JSONB, varchar, varchar, integer) cascade;
CREATE OR REPLACE FUNCTION public.mt_upsert_powerbiexportdocument(doc jsonb, docdotnettype character varying, docid character varying, revision integer)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
DECLARE
