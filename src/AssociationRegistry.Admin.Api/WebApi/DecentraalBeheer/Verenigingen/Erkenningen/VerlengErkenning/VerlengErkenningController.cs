...
public class VerlengErkenningController : ApiController
{
    private readonly IMessageBus _messageBus;
    private readonly IValidator<VerlengErkenningRequest> _validator;

    public VerlengErkenningController(IMessageBus messageBus, IValidator<VerlengErkenningRequest> validator)
    {
    ...
    /// <response code="202">De erkenning werd verlengd.</response>
    /// <response code="400">Er was een probleem met de doorgestuurde waarden.</response>
    /// <response code="412">De gevraagde vereniging heeft niet de verwachte sequentiewaarde.</response>
    /// <response code="500">Er is een interne fout opgetreden.</response>
    [HttpPost("{vCode}/erkenningen/{erkenningId}/verlengingen")]
    [ConsumesJson]
    [ProducesJson]
    [SwaggerRequestExample(typeof(VerlengErkenningRequest), typeof(VerlengErkenningRequestExamples))]
    ...
    public async Task<IActionResult> Post(
        ...
        [FromBody] VerlengErkenningRequest request,
    ...
    )
    {
        ...
        var envelope = new CommandEnvelope<VerlengErkenningCommand>(request.ToCommand(vCode, erkenningId, request.Einddatum, request.Hernieuwingsdatum), metaData);
