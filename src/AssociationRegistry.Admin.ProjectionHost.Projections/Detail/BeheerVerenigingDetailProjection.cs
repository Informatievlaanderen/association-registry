...
public partial class BeheerVerenigingDetailProjection : EventProjection
{
    public static readonly ShardName ShardName = new("beheer.postgres.detail");
    ...
    public async Task Project(IEvent<NaamWerdGewijzigd> @event, IDocumentOperations ops)
    {
        ...
        var gerelateerdeVerenigingen = await ops.Query<BeheerVerenigingDetailDocument>()
        ...
        {
            gerelateerdeVereniging.Relaties = gerelateerdeVereniging
                .Relaties.UpdateSingleWith(
                    identityFunc: relatie => relatie.AndereVereniging.VCode == @event.Data.VCode,
                    update: r => r with { AndereVereniging = r.AndereVereniging with { Naam = @event.Data.Naam } }
                )
                .ToArray();
    ...
    public async Task Project(IEvent<VertegenwoordigerWerdGewijzigdInKBO> @event, IDocumentOperations ops) =>
        await Update(@event, ops, BeheerVerenigingDetailProjector.Apply);
    ...
    public async Task Project(IEvent<SubverenigingRelatieWerdGewijzigd> @event, IDocumentOperations ops) =>
        await Update(@event, ops, BeheerVerenigingDetailProjector.Apply);
    ...
    public async Task Project(IEvent<ErkenningWerdGeregistreerd> @event, IDocumentOperations ops) =>
        await Update(@event, ops, BeheerVerenigingDetailProjector.Apply);
    ...
    private static async Task Update<T>(
        IEvent<T> @event,
        IDocumentOperations ops,
        Action<IEvent<T>, BeheerVerenigingDetailDocument> action
    )
        where T : notnull => await Update(@event.StreamKey!, @event, ops, action);
    ...
    private static async Task UpdateMetadataOnly<T>(IEvent<T> @event, IDocumentOperations ops)
        where T : notnull
    {
    ...
    private static BeheerVerenigingDetailDocument DoCreate<T>(
        IEvent<T> @event,
        IDocumentOperations ops,
        Func<IEvent<T>, BeheerVerenigingDetailDocument> action
    )
        where T : notnull
    {
