...
public class SchrijfVertegenwoordigersInMessageHandler
{
    ...
    public SchrijfVertegenwoordigersInMessageHandler(
        IAggregateSession aggregateSession,
        IMagdaGeefPersoonService magdaGeefPersoonService,
        ILogger<SchrijfVertegenwoordigersInMessageHandler> _logger
    )
    {
    ...
    {
        chain
            .OnException<MagdaException>()
            .ScheduleRetry(10.Seconds(), 1.Minutes(), 1.Hours(), 1.Days())
    ...
    )
    {
        ...
        await vereniging.SchrijfVertegenwoordigersIn(
            _magdaGeefPersoonService,
            envelope.Metadata,
            cancellationToken,
            _logger
        );
        ...
        _logger.LogInformation(
            $"SchrijfVertegenwoordigersInMessageHandler successful for VCode {envelope.Command.VCode}"
        );
