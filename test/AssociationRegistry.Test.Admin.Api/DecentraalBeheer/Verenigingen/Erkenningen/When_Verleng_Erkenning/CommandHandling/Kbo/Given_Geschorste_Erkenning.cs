...
public class Given_Geschorste_Erkenning
{
    ...
    public async ValueTask Then_Throw_VerlopenErkenningKanNietGeschorstWorden()
    {
        ...
        var commandEnvelope = CreateCommandEnvelopeWithSameInitiator(teVerlengenErkenningId);

        var exception = await Assert.ThrowsAsync<GeschorsteErkenningKanNietVerlengdWorden>(async () =>
        {
            await _commandHandler.Handle(commandEnvelope);
        });

        exception.Message.Should().Be(ExceptionMessages.GeschorsteErkenningKanNietVerlengdWorden);
    }

    private CommandEnvelope<VerlengErkenningCommand> CreateCommandEnvelopeWithSameInitiator(int teVerlengenErkenningId)
    {
        var command = _fixture.Create<VerlengErkenningCommand>() with
        {
            VCode = _scenario.VCode,
            Erkenning = _fixture.Create<TeVerlengenErkenning>() with { ErkenningId = teVerlengenErkenningId },
        };
