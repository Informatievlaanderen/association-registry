...
public class Given_A_Locatie_With_Adres_id
{
    ...
    public async ValueTask Then_A_LocatieWerdToegevoegd_Event_Is_Saved()
    {
        ...
        var commandHandler = new WijzigLocatieCommandHandler(
        ...
        grarClient
            .Setup(s => s.GetPostalInformationDetail(It.IsAny<string>()))
            .ReturnsAsync(
                fixture.Create<PostalInfoDetailResponse>() with
                {
        ...
        await commandHandler.Handle(
            new CommandEnvelope<WijzigLocatieCommand>(command, fixture.Create<CommandMetadata>())
        );

        verenigingRepositoryMock.ShouldHaveSavedExact(
            new LocatieWerdGewijzigd(
                EventFactory.Locatie(
                    Locatie.Hydrate(
                        locatie.LocatieId,
                        locatie.Naam,
                        isPrimair: false,
