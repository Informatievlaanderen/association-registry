...
public class With_A_Einddatum
{
    ...
    public With_A_Einddatum()
    {
        ...
        _command = new StopVerenigingCommand(
            _scenario.VCode,
        ...
        var commandHandler = new StopVerenigingCommandHandler(
            _aggregateSessionMock,
            new ClockStub(_command.Einddatum.Value.AddDays(1))
        );

        commandHandler
            .Handle(new CommandEnvelope<StopVerenigingCommand>(_command, commandMetadata), CancellationToken.None)
            .GetAwaiter()
            .GetResult();
    }
    ...
    {
        _aggregateSessionMock.ShouldHaveSavedExact(new VerenigingWerdGestopt(_command.Einddatum.Value));
    }
}
