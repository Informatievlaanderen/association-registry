...
public class DetailVerenigingResponseTemplate : ResponseTemplate
{
    ...
    public DetailVerenigingResponseTemplate()
        : base("templates.adminApi.DetailVerenigingResponse.json")
    {
    ...
    public DetailVerenigingResponseTemplate WithLocatieWithAdres(
        int locatieId,
        string type,
        string naam,
    ...
    public DetailVerenigingResponseTemplate WithLocatieWithAdresId(
        int locatieId,
        string type,
        string naam,
        string broncode,
    ...
    public DetailVerenigingResponseTemplate FromRequest(RegistreerFeitelijkeVerenigingRequest request, string vCode)
    {
        WithVCode(vCode)
           .WithJsonLdType(JsonLdType.FeitelijkeVereniging)
           .WithType(Verenigingstype.FeitelijkeVereniging)
           .WithNaam(request.Naam)
           .WithKorteNaam(request.KorteNaam)
           .WithKorteBeschrijving(request.KorteBeschrijving)
           .WithStartdatum(request.Startdatum)
           .WithIsUitgeschrevenUitPubliekeDatastroom(request.IsUitgeschrevenUitPubliekeDatastroom)
           .WithDoelgroep(vCode, request.Doelgroep.Minimumleeftijd.Value, request.Doelgroep.Maximumleeftijd.Value)
           .WithBron(Bron.Initiator);

        foreach (var h in request.HoofdactiviteitenVerenigingsloket)
        {
            WithHoofdactiviteit(h, HoofdactiviteitVerenigingsloket.Create(h).Naam);
        }

        foreach (var (c, i) in request.Contactgegevens.Select((c, i) => (c, i + 1)))
        {
            WithContactgegeven(i, Bron.Initiator, c.Contactgegeventype, c.Waarde, vCode, c.Beschrijving, c.IsPrimair);
        }

        foreach (var (l, i) in request.Locaties.Select((l, i) => (l, i + 1)))
        {
            WithLocatie(i, l, Bron.Initiator, vCode);
        }

        foreach (var (v, i) in request.Vertegenwoordigers.Select((l, i) => (l, i + 1)))
        {
            WithVertegenwoordiger(i, v.Voornaam, v.Achternaam, v.Roepnaam, v.Rol, v.Insz, v.Email, v.Telefoon, v.Mobiel,
                                  v.SocialMedia, v.IsPrimair, Bron.Initiator, vCode);
        }
    ...
    private DetailVerenigingResponseTemplate WithLocatie(int locatieId, ToeTeVoegenLocatie l, Bron bron, string vCode)
    {
        if (l.Adres is not null && l.AdresId is null)
