...
{
    public static async Task<VerenigingenPerInszResponse> GetVerenigingenPerInsz(
        this IAlbaHost source,
        VerenigingenPerInszRequest request,
        long? expectedSequence
    )
    {
        await WaitForExpectedSequence(source, expectedSequence, VerenigingenPerInszProjection.ShardName.Identity);

        return await source
            .PostJson(request, $"/v1/verenigingen", JsonStyle.Mvc)
    ...
    private static async Task WaitForExpectedSequence(IAlbaHost source, long? expectedSequence, string projectionName)
    {
        ...
        var result = (await store.Advanced.AllProjectionProgress())
            .SingleOrDefault(x => x.ShardName == projectionName)
            ?.Sequence;
