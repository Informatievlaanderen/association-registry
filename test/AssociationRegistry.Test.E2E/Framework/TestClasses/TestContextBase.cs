    ...
    where TScenario : IScenario
{
    ...
    private async Task WaitBeforeCommands(FullBlownApiSetup setup, long max, IAlbaHost host, string beforeOrAfter)
    {
        ...
        setup.Logger.LogWarning(
            "{BEFOREORAFTER} COMMANDS: Waiting for high water mark of {HighWaterMark} on database schema {DatabaseSchemaName}",
            beforeOrAfter,
            highWaterMark,
            databaseSchemaName
        );
        ...
        setup.Logger.LogWarning(
            "{BEFOREORAFTER} COMMANDS: Projection progress for {DatabaseSchemaName} is {ProjectionProgress}",
            beforeOrAfter,
            databaseSchemaName,
            projectionProgress
        );
        ...
        var counter = 0;
        while (counter < 10 && !projectionProgress.All(x => x.Sequence >= highWaterMark))
        {
            setup.Logger.LogWarning(
                $"{beforeOrAfter} COMMANDS: WAITING FOR ANY projection progress for: "
        ...
        if (!projectionProgress.All(x => x.Sequence >= highWaterMark))
        {
            ...
            setup.Logger.LogCritical(
                $"{beforeOrAfter} COMMANDS: Projection progress for {databaseSchemaName} was not high enough:"
                    + string.Join(
                        ", ",
                        projectionProgress
                            .Where(x => x.Sequence <= highWaterMark)
                            .Select(x => $"{x.ShardName}: {x.Sequence}\n")
                    )
            );
        }
        ...
        {
            setup.Logger.LogCritical(
                $"{beforeOrAfter} COMMANDS: Projection progress for {databaseSchemaName} was high enough!"
            );
        }
    }

    private async Task<IReadOnlyList<ShardState>> UpdateProjectionProgress(
    ...
    )
    {
        ...
        setup.Logger.LogWarning(
            "COMMANDS: Waiting for projection progress to be at least {HighWaterMark} on database schema {DatabaseSchemaName}",
            highWaterMark,
            databaseSchemaName
        );
        ...
        setup.Logger.LogWarning(
            "COMMANDS: Projection progress for {DatabaseSchemaName} is {ProjectionProgress}",
            databaseSchemaName,
            projectionProgress
        );
