...
public class VerwijderBankrekeningnummerRequestFactory : ITestRequestFactory<NullRequest>
{
    ...
    public async Task<CommandResult<NullRequest>> ExecuteRequest(IApiSetup apiSetup)
    {
        var response = (
            ...
            {
                s.Delete.Url(
                    $"/v1/verenigingen/{_scenario.VerenigingZonderEigenRechtspersoonlijkheidWerdGeregistreerd.VCode}/bankrekeningnummers/{_scenario.BankrekeningnummerWerdToegevoegd.BankrekeningnummerId}"
                );
