    ...
    where TScenario : IScenario, new()
{
    ...
    protected override async Task RefreshProjectionsAsync(IProjectionDaemon daemon) =>
        await daemon.RebuildProjectionAsync(BeheerVerenigingDetailProjection.ShardName.Name, CancellationToken.None);

    protected override async Task<BeheerVerenigingDetailDocument> GetResultAsync(
        IDocumentSession session,
        TScenario scenario
    ) => await session.Query<BeheerVerenigingDetailDocument>().SingleAsync(x => x.VCode == scenario.AggregateId);
}

public class BeheerDetailScenarioClassFixture<TScenario> : IClassFixture<BeheerDetailScenarioFixture<TScenario>>
    where TScenario : IScenario, new() { }
