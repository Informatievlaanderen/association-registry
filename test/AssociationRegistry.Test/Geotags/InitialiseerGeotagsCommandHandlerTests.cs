...
public class InitialiseerGeotagsCommandHandlerTests
{
    ...
    {
        var scenario =
            new VerenigingZonderEigenRechtspersoonlijkheidWerdGeregistreerdZonderGeotagsInitialisatieScenario();
        ...
        var verenigingRepository = new AggregateSessionMock(
            scenario.GetVerenigingState(),
            expectedLoadingDubbel: true,
            expectedLoadingVerwijderd: true
        );

        var (geotagsService, geotags) = Faktory
            .New()
            .GeotagsService.ReturnsRandomGeotags(
                locaties.Select(Locatie.Hydrate),
                werkingsgebieden.Select(x => Werkingsgebied.Hydrate(x.Code, x.Naam)).ToArray()
            );

        var sut = new InitialiseerGeotagsCommandHandler(verenigingRepository, geotagsService.Object);

        await sut.Handle(
            new CommandEnvelope<InitialiseerGeotagsCommand>(
                new InitialiseerGeotagsCommand(scenario.VCode),
