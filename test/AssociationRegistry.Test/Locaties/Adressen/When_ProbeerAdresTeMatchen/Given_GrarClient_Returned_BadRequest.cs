...
public class Given_GrarClient_Returned_BadRequest
{
    ...
    public async Task Then_AdresKonNietOvergenomenWordenUitAdressenregister(Type verenigingType)
    {
        ...
        await handler.Handle(
            new ProbeerAdresTeMatchenCommand(verenigingWerdGeregistreerd.VCode, firstRegisteredLocatie.LocatieId)
        );
        ...
        @event
            .Adres.Should()
            .BeEquivalentTo(
                $"{firstRegisteredLocatie.Adres.Straatnaam} {firstRegisteredLocatie.Adres.Huisnummer}"
                    + (
