...
public class With_AddressId_Different_In_Response_And_Location
{
    ...
    public With_AddressId_Different_In_Response_And_Location()
    {
        ...
        _aggregateSessionMock = factory.AggregateSession.Mock(_state, expectedLoadingDubbel: true);
    ...
    public async ValueTask Then_ItClearsBothAddressAndAddressId()
    {
        ...
        _aggregateSessionMock.ShouldHaveSavedExact(addedEvents);

        var stateWithAddedEvents = addedEvents.Aggregate(_state, (state, @event) => state.Apply((dynamic)@event));
        var repositoryMockWithAddedEvents = Faktory
            .New()
            .AggregateSession.Mock(stateWithAddedEvents, expectedLoadingDubbel: true);
        var commandHandlerWithAddedEvents = new SyncAdresLocatiesCommandHandler(
            repositoryMockWithAddedEvents,
            _grarClientMock.Object,
            new NullLogger<SyncAdresLocatiesCommandHandler>(),
            _geotagsService.Object
        );
